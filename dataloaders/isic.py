...
class DatasetISIC(Dataset):
    def __init__(self, datapath, split, shot=1): # Default: n_shot = 1
        self.split = split
        self.benchmark = 'isic'
        self.shot = shot
        ...
        print(f"Found {len(self.img_metadata)} images in {self.base_path}")
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_img = Image.open(query_name).convert('L')
        query_img = np.array(query_img)
        query_img = np.stack([query_img] * 3, axis=-1)  # 灰度图重复三次构造RGB样式
        query_img = Image.fromarray(query_img)

        support_imgs = []
        for name in support_names:
            support_img = Image.open(name).convert('L')
            support_img = np.array(support_img)
            support_img = np.stack([support_img] * 3, axis=-1)  # 灰度图重复三次
            support_imgs.append(Image.fromarray(support_img))
